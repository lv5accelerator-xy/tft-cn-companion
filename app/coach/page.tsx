        ...
        {guidance.map((match, index) => {
          ...
          return <article className={`${styles.card} ${index === 0 ? styles.best : ""}`} key={`${match.comp.sourceId}:${match.comp.id}`}>
            ...
            <div className={styles.body}>
              <div className={styles.title}><div><h2>{locale === "zh" ? match.comp.nameZh : match.comp.name}</h2><p>{match.comp.source} · {match.comp.playstyle} · Tier {match.comp.sourceTier ?? match.comp.ranking?.sourceTier ?? match.comp.tier}</p></div><span>{match.comp.difficulty}</span></div>
