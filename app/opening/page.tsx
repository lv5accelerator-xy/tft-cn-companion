      ...
      <section className={styles.results}>
        ...
        {!hasSignals ? <div className={styles.empty}>{tr("例如：凯尔 ×2 + 大剑 + 拳套 + 反曲弓。系统会立刻重排阵容。", "Example: Kayle ×2 + B.F. Sword + Sparring Gloves + Recurve Bow. Results rerank instantly.")}</div> : <div className={styles.resultList}>{topMatches.map((match, index) => {
          ...
          return <article className={styles.resultCard} key={`${match.comp.sourceId}-${match.comp.id}`}>
            ...
            <div className={styles.resultBody}><div className={styles.resultTitle}><div><h3>{locale === "zh" ? match.comp.nameZh : match.comp.name}</h3><p>{match.comp.source} · {match.comp.playstyle} · {freshness.stale ? "⚠ " : ""}{freshness.label}</p></div><span className={styles[`tier${match.comp.tier}`]}>{match.comp.sourceTier ?? match.comp.ranking?.sourceTier ?? match.comp.tier}</span></div>
