...
function CompRow({ comp, champions }: { comp: UnifiedMetaComp; champions: CatalogEntry[] }) {
  ...
  return (
    <article className={styles.row} onDoubleClick={() => setExpanded((value) => !value)}>
      ...
      <div className={styles.actions}><button className={styles.focusButton} onClick={openFocus}>◉ {tr("对局模式", "Focus")}</button><button className={styles.builderButton} onClick={openBuilder}>Builder</button></div>
