    ...
    <div className={styles.page}>
      ...
      <section className={styles.toolbar}>
        <input
          ...
          placeholder={tr("搜索英雄 / 羁绊 / 技能", "Search champion / trait / ability")}
        />
        <div className={styles.costs}>
          <button className={cost === null ? styles.active : ""} onClick={() => setCost(null)}>{tr("全部", "All")}</button>
          {[1, 2, 3, 4, 5].map((value) => (
            <button key={value} className={cost === value ? styles.active : ""} onClick={() => setCost(value)}>{value} {tr("费", "Cost")}</button>
          ))}
        </div>
      ...
      <div className={styles.hint}>
        ...
        {detailsLoading
          ? tr("正在载入 CommunityDragon 当前版本英雄详情…", "Loading current-patch champion details from CommunityDragon…")
          : tr("悬停英雄头像：显示技能全文、羁绊、生命、法力、攻击力、攻速、护甲、魔抗、射程与暴击。", "Hover a champion portrait for full ability text, traits, health, mana, AD, AS, armor, MR, range and crit.")}
      </div>
      ...
      <section className={styles.tableWrap}>
        <table className={styles.table}>
          <thead>
            <tr>
              <th>#</th>
              <th>{tr("英雄", "Champion")}</th>
              <th>{tr("费用", "Cost")}</th>
              <th>{tr("羁绊", "Traits")}</th>
              <th>{tr("技能", "Ability")}</th>
              <th>{tr("基础属性", "Base stats")}</th>
            </tr>
          </thead>
          <tbody>
            {champions.map((champion, index) => {
              const detail = detailForEntry(details, champion);
              const mana = detail?.stats?.mana !== undefined
                ? `${stat(detail.stats.initialMana ?? 0)}/${stat(detail.stats.mana)}`
                : "—";
              return (
                <tr key={champion.id}>
                  ...
                  <td>
                    <div className={styles.champion}>
                      ...
                      <div>
                        <strong>{nameOf(champion)}</strong>
                        <span>{secondaryNameOf(champion)}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className={styles.traits}>
                      {(detail?.traits ?? []).map((name) => <span key={name}>{name}</span>)}
                      {!detailsLoading && !detail ? <em>—</em> : null}
                    </div>
                  </td>
                  <td>
                    <div className={styles.ability}>
                      <strong>{detail?.abilityName || (detailsLoading ? tr("载入中…", "Loading…") : "—")}</strong>
                      {detail?.abilityDesc ? <span>{detail.abilityDesc}</span> : null}
                    </div>
                  </td>
                  <td>
                    <div className={styles.stats}>
                      <span><b>HP</b> {stat(detail?.stats?.hp)}</span>
                      <span><b>AD</b> {stat(detail?.stats?.damage)}</span>
                      <span><b>AS</b> {stat(detail?.stats?.attackSpeed)}</span>
                      <span><b>Mana</b> {mana}</span>
                      <span><b>AR</b> {stat(detail?.stats?.armor)}</span>
                      <span><b>MR</b> {stat(detail?.stats?.magicResist)}</span>
                    </div>
                  </td>
                </tr>
              );
