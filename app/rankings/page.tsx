...
export default function RankingsPage() {
  ...
  useEffect(() => { const value = new URLSearchParams(window.location.search).get("source"); if (rankingSources.some(item => item.id === value)) setSource(value!); }, []);
  ...
  return <div className={styles.page}>
    ...
    <p className={styles.notice}>{tr("这是核验后的静态快照。Academy 当前 S 级为空；X 表示条件阵容。OP.GG 对照数据仍为 18.2，其他榜单为 18.2b。", "This is a verified static snapshot. Academy currently has no S tier; X means situational. OP.GG comparisons remain on 18.2, while other rankings use 18.2b.")} <a href="https://tactics.tools/team-compositions" target="_blank" rel="noreferrer">{tr("去 tactics.tools 交叉核验", "Cross-check on tactics.tools")} ↗</a></p>
    <div className={styles.filters}><label>{tr("来源", "Source")}<select value={source} onChange={event => setSource(event.target.value)}><option value="all">{tr("全部来源", "All sources")}</option>{rankingSources.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>{tr("原始评级", "Original tier")}<select value={tier} onChange={event => setTier(event.target.value)}>{["all", "S", "A", "B", "C", "D", "X"].map(value => <option value={value} key={value}>{value === "all" ? tr("全部评级", "All tiers") : value}</option>)}</select></label><label>{tr("搜索阵容／英雄", "Search comps / units")}<input value={query} onChange={event => setQuery(event.target.value)} /></label><span>{groups.length} {tr("组", "groups")}</span></div>
