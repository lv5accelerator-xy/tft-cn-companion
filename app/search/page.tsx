  ...
  return (
    <div className={styles.page}>
      ...
      {!activeQuery ? <div className={styles.empty}>{tr("输入关键词开始搜索。", "Enter a keyword to search.")}</div> : <>
        {compResults.length > 0 && <section className={styles.section}><div className={styles.sectionHead}><strong>{tr("阵容", "Comps")}</strong><span>{compResults.length} {tr("条结果", "results")}</span></div><div className={styles.grid}>{compResults.map((comp) => <div className={styles.compCard} key={`${comp.sourceId}-${comp.id}`}><strong>{locale === "zh" ? comp.nameZh : comp.name}</strong><span>{locale === "zh" ? comp.name : comp.nameZh}</span><span>{comp.source} · {comp.playstyle} · {comp.sourceTier ?? comp.ranking?.sourceTier ?? comp.tier}</span></div>)}</div></section>}
