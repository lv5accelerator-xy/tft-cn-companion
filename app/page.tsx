...
export default function HomePage() {
  ...
  const championByName = useMemo(() => {
    ...
    champions.forEach((champion) => {
      [champion.id, champion.nameEn, champion.nameZh, ...(champion.aliases ?? [])].forEach((name) => { if (name) map.set(normalize(name), champion); });
    });
    return map;
  }, [champions]);
  ...
  const personalComps = useMemo(() => {
    ...
    favorites.forEach((ref) => {
      const comp = byRef.get(compRefKey(ref));
      if (comp && !used.has(compRefKey(comp))) { used.add(compRefKey(comp)); result.push({ comp, kind: "favorite" }); }
    });
    recents.forEach((ref) => {
      const comp = byRef.get(compRefKey(ref));
      if (comp && !used.has(compRefKey(comp))) { used.add(compRefKey(comp)); result.push({ comp, kind: "recent" }); }
    });
  ...
  return (
    <div className={styles.page}>
      <section className={styles.hero}>
        <div><div className={styles.eyebrow}>V1.3 · SECOND SCREEN WORKFLOW</div><h1>{tr("美服云顶中文副屏助手", "NA TFT companion for fast decisions")}</h1><p>{tr("打开网站先继续上一局，或者从候选阵容直接进入。每套候选会保留自己的 Stage 与镜像状态。", "Resume the last game first, or jump straight into a candidate. Each candidate keeps its own stage and mirror state.")}</p></div>
        <div className={styles.heroActions}><Link className={styles.primaryButton} href="/comps">{tr("开始新对局", "Start a new game")}</Link><Link className={styles.secondaryButton} href="/review">{tr("赛后复盘", "Review Center")}</Link></div>
      </section>
      ...
      <div className={styles.mainGrid}>
        <section className={styles.panel}>
          <div className={styles.panelHead}><div><h2>{tr("本局候选", "Game Candidates")}</h2><p>{tr("从首页直接进入 1 / 2 / 3 槽位；切换回来会恢复各自进度。", "Open slot 1 / 2 / 3 here; returning to a candidate restores its progress.")}</p></div><Link href="/comps">{tr("管理候选", "Manage")}</Link></div>
          <div className={styles.moduleGrid}>{[0, 1, 2].map((index) => {
            const comp = trayComps[index];
            const ref = tray[index];
            if (!comp || !ref) return <Link className={styles.moduleCard} href="/comps" key={index}><b>{index + 1} · {tr("空候选槽", "Empty candidate")}</b><span>{tr("从阵容库或开局助手加入一套阵容。", "Add a comp from the library or opening assistant.")}</span></Link>;
          ...
          <div className={styles.shortcutTip}><kbd>1</kbd><kbd>2</kbd><kbd>3</kbd><strong>{tr("进入 Focus 后可直接切换候选；按 ? 查看全部快捷键", "Switch candidates directly in Focus; press ? for all shortcuts")}</strong></div>
        </section>

        <section className={styles.panel}>
          <div className={styles.panelHead}><div><h2>{tr("快速工具", "Quick Tools")}</h2><p>{tr("高频操作放在第一屏。", "High-frequency actions stay on the first screen.")}</p></div></div>
          <div className={styles.moduleGrid}>
            <Link className={styles.moduleCard} href="/comps"><b>◆ {tr("阵容库", "Comps")}</b><span>{tr("收藏阵容、加入候选并查看数据更新时间。", "Favorite comps, pin candidates and see data freshness.")}</span></Link>
      ...
      {personalComps.length ? <section className={styles.panel}>
        <div className={styles.panelHead}><div><h2>{tr("收藏与最近使用", "Favorites & Recent")}</h2><p>{tr("减少重复搜索，常玩的阵容从这里直接继续。", "Skip repeated searching and reopen the comps you use most.")}</p></div><Link href="/comps">{tr("全部阵容", "All comps")}</Link></div>
        <div className={styles.compList}>{personalComps.map(({ comp, kind }) => {
          const units = compUnits(comp);
      ...
      <section className={styles.panel}>
        <div className={styles.panelHead}><div><h2>{tr("本版本推荐", "Recommended Comps")}</h2><p>{tr("每条攻略显示最近更新时间，过旧来源会给出提醒。", "Every recommendation shows freshness; stale sources are flagged.")}</p></div><Link href="/comps">{tr("全部阵容", "All comps")}</Link></div>
