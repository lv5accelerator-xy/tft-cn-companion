...
export default function ChampionHoverCard({ entry, children }: { entry: CatalogEntry; children: ReactNode }) {
  ...
  const statRows = useMemo(() => {
    ...
    if (!stats) return [];
    const mana = stats.mana !== undefined ? `${formatNumber(stats.initialMana ?? 0)} / ${formatNumber(stats.mana)}` : "—";
    return [
    ...
    <aside className={`${styles.card} ${styles[position.side]}`} style={{ top: position.top, left: position.left }} role="tooltip">
      <div className={styles.header}>
        <div className={`${styles.portrait} ${costClass(detail?.cost ?? entry.tier)}`}>
          {entry.imageUrl ? <Image src={entry.imageUrl} alt={entry.nameEn} width={62} height={62} unoptimized /> : <span>{nameOf(entry).slice(0, 1)}</span>}
        </div>
        <div className={styles.identity}>
          ...
          <div className={styles.traits}>{(detail?.traits ?? []).map((trait) => <span key={trait}>{trait}</span>)}</div>
        </div>
      </div>
        ...
        <>
          ...
          <div className={styles.ability}>
            <div className={styles.abilityHead}>
              ...
              <div><span>{tr("技能", "Ability")}</span><strong>{detail.abilityName || tr("当前数据未提供技能名", "Ability name unavailable")}</strong></div>
            </div>
