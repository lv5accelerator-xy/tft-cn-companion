...
import EndGameReviewButton from "./EndGameReviewButton";
import TrainingFocusStrip from "./TrainingFocusStrip";
...
function NavLinks({ items, pathname }: { items: NavItem[]; pathname: string }) {
  ...
  return items.map((item) => <Link key={item.href} href={item.href} className={isActive(pathname, item.href) ? styles.active : ""} title={locale === "zh" ? item.zh : item.en}><span className={styles.glyph}>{item.glyph}</span><span className={styles.navLabel}>{locale === "zh" ? item.zh : item.en}</span></Link>);
}
...
export default function DesktopShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved === "1" || saved === "0") setCollapsed(saved === "1"); else setCollapsed(autoCompact);
    } catch { setCollapsed(autoCompact); }
  }, []);
  ...
  return <div className={`${styles.app} ${collapsed ? styles.collapsed : ""}`} data-ui-shell data-ui-route={route} data-command-ui="neon-command">
    <aside className={styles.sidebar} data-ui-sidebar>
      ...
      <div className={styles.sectionLabel} data-ui-section-label>{tr("版本资料", "META")}</div><nav className={styles.nav} aria-label="TFT reference"><NavLinks items={metaNav} pathname={pathname} /></nav>
      <div className={styles.sectionLabel} data-ui-section-label>{tr("工具", "TOOLS")}</div><nav className={styles.nav} aria-label="TFT tools"><NavLinks items={toolNav} pathname={pathname} /></nav>
      <Link href="/sources" className={styles.sidebarFoot} title={tr("数据来源状态", "Source status")}><span className={styles.dot} /><div><strong>{tr("数据源在线", "Sources online")}</strong><span>Riot · Meta feeds</span></div></Link>
    </aside>

    <div className={styles.workspace}>
      <header className={styles.topbar} data-ui-topbar><QuickSearch /><FocusPlanTrail pathname={pathname} /><div className={styles.topActions} data-ui-top-actions><AppExperience pathname={pathname} /><DensityControl /><ShortcutHelp pathname={pathname} />{route === "focus" ? <EndGameReviewButton /> : null}<SessionResetButton /><button className={styles.language} onClick={toggleLocale} title={tr("切换到英文", "Switch to Chinese")}>{locale === "zh" ? "中 / EN" : "EN / 中"}</button><span className={styles.region} data-ui-secondary-status data-ui-tip={tr("北美服务器", "North America server")}>NA</span><span className={styles.patch} data-ui-secondary-status data-ui-tip={tr("当前资料版本 Patch 18.2", "Current data patch 18.2")}>18.2</span><span className={styles.version} data-ui-version-status data-ui-tip={tr("个人复盘 → 5 局训练闭环", "Personal review → five-game practice loop")}>V1.6.9</span></div></header>
      <main className={styles.content} data-ui-content>{route === "review" ? <ReviewCaptureBridge /> : null}{route === "home" ? <TrialWelcome /> : null}{route === "review" ? <ReviewMaintenanceBar /> : null}{route === "review" ? <ReviewPulse /> : null}{route === "focus" || route === "review" ? <TrainingFocusStrip mode={route === "focus" ? "focus" : "review"} /> : null}{children}</main>
    </div>
  </div>;
}
