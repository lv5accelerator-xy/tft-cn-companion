...
export default function DesktopShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`${styles.app} ${collapsed ? styles.collapsed : ""}`} data-ui-shell data-ui-route={route} data-command-ui="neon-command">
    ...
    <div className={styles.workspace}>
      <header className={styles.topbar} data-ui-topbar><QuickSearch /><FocusPlanTrail pathname={pathname} /><div className={styles.topActions} data-ui-top-actions><AppExperience pathname={pathname} /><DensityControl /><ShortcutHelp pathname={pathname} />{route === "focus" ? <EndGameReviewButton /> : null}<SessionResetButton /><button className={styles.language} onClick={toggleLocale} title={tr("切换到英文", "Switch to Chinese")}>{locale === "zh" ? "中 / EN" : "EN / 中"}</button><span className={styles.region} data-ui-secondary-status data-ui-tip={tr("北美服务器", "North America server")}>NA</span><span className={styles.patch} data-ui-secondary-status data-ui-tip={tr("当前资料版本 Patch 18.2", "Current data patch 18.2")}>18.2</span><span className={styles.version} data-ui-version-status data-ui-tip={tr("18.2 数据一致性与整局流程优化", "18.2 data fidelity and full-game flow polish")}>V1.6.8</span></div></header>
      <main className={styles.content} data-ui-content>{route === "review" ? <ReviewCaptureBridge /> : null}{route === "home" ? <TrialWelcome /> : null}{route === "review" ? <ReviewMaintenanceBar /> : null}{route === "review" ? <ReviewPulse /> : null}{children}</main>
    </div>
  </div>;
}
