...
import Ajv from "ajv";
import { AnalysisAccessError, analysisAccessConfigured, authenticateAnalysis, consumeAnalysisQuota } from "@/lib/image-analysis-access";
import { AnalysisInputError, MAX_IMAGES, readAnalysisInput } from "@/lib/image-analysis-input";
...
export async function GET() {
  return NextResponse.json({
    configured: Boolean(process.env.OPENAI_API_KEY?.trim()) && analysisAccessConfigured(),
    model: MODEL,
...
function parseStructuredOutput(payload: OpenAIResponse) {
  ...
  try {
    ...
    if (!validateOutput(result)) throw new Error("Invalid analysis output schema");
    return result;
  } catch {
...
const validateOutput = new Ajv({ strict: false }).compile(schema);

class UpstreamError extends Error {
  constructor(public retryable: boolean) { super("Image analysis upstream failed."); }
}
...
async function callOpenAI(
  ...
  if (!response.ok) {
    ...
    throw new UpstreamError(response.status === 429 || response.status >= 500);
  }
...
export async function POST(request: Request) {
  ...
  let input: Awaited<ReturnType<typeof readAnalysisInput>>;
  try {
    const userId = await authenticateAnalysis(request);
    input = await readAnalysisInput(request);
    await consumeAnalysisQuota(userId);
  } catch (error) {
    if (error instanceof AnalysisInputError || error instanceof AnalysisAccessError) {
      return NextResponse.json({ error: error.message }, { status: error.status,
        headers: error instanceof AnalysisAccessError && error.retryAfter ? { "Retry-After": String(error.retryAfter) } : undefined });
    }
    return NextResponse.json({ error: "Unable to process this request." }, { status: 400 });
  }
  const { images, sourceName, patch } = input;
  ...
  for (const model of attempts) {
    try {
      ...
      if (remaining <= 0 || request.signal.aborted) break;
      const signal = AbortSignal.any([request.signal, AbortSignal.timeout(Math.min(22000, remaining))]);
      const { payload } = await callOpenAI(apiKey, model, content, signal);
  ...
  return NextResponse.json(
    {
      error: "AI 图片分析暂时失败，请稍后重试。 / Image analysis temporarily failed. Please try again later.",
    },
    { status: 502 },
  );
}
