...
export default function SharePage() {
  ...
  function buildCanvas() {
    ...
    if (mode === "review" && review) {
      ...
      ctx.fillText(`Tier ${comp.sourceTier ?? comp.ranking?.sourceTier ?? comp.tier}`, 862, 282); ctx.font = "600 16px Arial, sans-serif"; ctx.fillStyle = "#c7cbd5";
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.preview}><div className={styles.previewHead}><span>{mode === "review" ? "POST-GAME REVIEW" : "COMP SHARE"}</span><strong>{title}</strong><small>{subtitle}</small></div><div className={styles.previewBody}><div className={styles.board}>{[0,1,2,3].flatMap((row) => [0,1,2,3,4,5,6].map((col) => { const unit = board.find((position) => position.row === row && position.col === col); return <div className={styles.cell} key={`${row}-${col}`}>{unit ? <span>{shortUnit(unit.unit)}</span> : null}</div>; }))}</div><aside>{mode === "review" && review ? <><b>#{review.placement}</b><span>Lv.{review.level}{review.endStage ? ` · ${review.endStage}` : ""}</span><p>{review.tags.length ? review.tags.map((tag) => reviewTagLabel(tag, "zh")).join(" · ") : "无问题标签"}</p><small>{review.note || "本局未填写额外备注"}</small></> : comp ? <><b>Tier {comp.sourceTier ?? comp.ranking?.sourceTier ?? comp.tier}</b><span>Patch {comp.patch}</span><p>{comp.coreUnits.slice(0,4).join(" · ")}</p><small>{comp.source} · {comp.playstyle}</small></> : null}</aside></div><footer>TFT CN Companion · {PRODUCT_URL}</footer></section>
