...
async function request(path) {
  ...
  try { return await fetch(`${baseUrl}${path}`, { redirect: "follow", signal: controller.signal, headers: { "user-agent": "tft-cn-companion-release-smoke/1.6.5" } }); }
...
for (const path of requiredRoutes) {
  try {
    ...
    if (path === "/api/health") {
      ...
      if (payload?.status !== "ok" || payload?.app !== "tft-cn-companion") { console.error(`Smoke failed: ${path} returned an invalid health payload.`); failed = true; continue; }
    }
