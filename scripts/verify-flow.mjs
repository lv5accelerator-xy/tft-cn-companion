...
const checks = [
  ...
  ["app/components/DesktopShell.tsx", ["ReviewCaptureBridge", "EndGameReviewButton", "V1.6.8"]],
  ...
  ["app/insights/page.tsx", ["这套阵容暂无个人复盘", "hasOlder", "!map.has(compFilter)"]],
  ["app/insights/page.tsx", ["V1.6.8", "source", "id", "WindowSize"]],
  ["scripts/verify-meta.mjs", ["Stage 2/3/4 contract", "opening rule coverage", "Duplicate board hex", "canonicalOpeningRuleId"]],
  ["public/sw.js", ["v1.6.8-set18", "/review/history", "/insights"]],
  ["app/api/health/route.ts", ["version: \"1.6.8\""]],
];
...
for (const [file, needles] of checks) {
  if (!fs.existsSync(file)) {
    console.error(`V1.6.8 flow verification failed: missing ${file}`);
    failed = true;
    continue;
  }
  ...
  for (const needle of needles) {
    if (!source.includes(needle)) {
      console.error(`V1.6.8 flow verification failed: ${file} is missing ${needle}`);
      failed = true;
    }
  }
}
...
for (const id of requiredCompIds) {
  if (!snapshotIds.has(id)) {
    console.error(`V1.6.8 flow verification failed: reviewed baseline is missing ${id}`);
    failed = true;
  }
}
if (snapshotIds.size !== requiredCompIds.length) {
  console.error(`V1.6.8 flow verification failed: expected ${requiredCompIds.length} reviewed comp IDs, got ${snapshotIds.size}`);
  failed = true;
}

const pkg = JSON.parse(fs.readFileSync("package.json", "utf8"));
if (pkg.version !== "1.6.8") {
  console.error(`V1.6.8 flow verification failed: package version is ${pkg.version}`);
  failed = true;
}
if (!pkg.scripts?.["verify:flow"] || !pkg.scripts?.["release:check"]?.includes("verify:flow")) {
  console.error("V1.6.8 flow verification failed: verify:flow must be part of release:check.");
  failed = true;
}

if (failed) process.exit(1);
console.log("V1.6.8 flow contract verified: exact 14-comp 18.2 baseline, canonical opening-rule aliases, explicit Focus → Review capture, data-fidelity guards, per-comp Insights, offline shell and release version wiring are present.");
